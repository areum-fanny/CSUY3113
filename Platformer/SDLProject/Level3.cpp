#include "Level3.h"
#define LEVEL3_WIDTH 26
#define LEVEL3_HEIGHT 8
#define LEVEL3_ENEMY_COUNT 2
#define LEVEL3_WATER_COUNT 15

unsigned int level3_data[] =
{
21,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,21,
21,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,21,
21,12,12,12,12,12,14,14,14,14,14,12,12,12,12,12,12,12,12,12,12,12,12,12,12,21,
21,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,21,
...
void Level3::Initialize() {
    ...
    state.key->position = glm::vec3(8, -1, 0);
    state.key->type = KEY;
    ...
    //Water
    x = 1.0f;
    state.obstacles = new Entity[LEVEL3_WATER_COUNT];
    GLuint waterTextureID = Util::LoadTexture("water.png");
    for (int i = 0; i < LEVEL3_WATER_COUNT;i++) {
        if (i == 2) x = 7;
        if (i == 4) x = 11;
        state.obstacles[i].textureID = waterTextureID;
        state.obstacles[i].position = glm::vec3(x, -7.5f, 0);
    ...
    GLuint enemyTextureID = Util::LoadTexture("zombies-male.png");
    ...
    state.enemies[0].enemylevel = ROUND3;

    state.enemies[1].textureID = enemyTextureID;
    state.enemies[1].position = glm::vec3(14, -2, 0);
    state.enemies[1].acceleration = glm::vec3(0, -9.81f, 0);
    state.enemies[1].animLeft = new int[4]{ 13,12,13,14 };
    state.enemies[1].animRight = new int[4]{ 25,24,25,26 };
    state.enemies[1].animCols = 12;
    state.enemies[1].animRows = 8;
    state.enemies[1].animFrames = 4;
    state.enemies[1].animIndices = state.enemies[0].animLeft;
    state.enemies[1].animIndex = 0;
    state.enemies[1].speed = 0.2f;
    state.enemies[1].type = ENEMY;
    state.enemies[1].aiType = WALKER2;
    state.enemies[1].aiState = WALKING;
    state.enemies[1].speed = 1.0f;
    state.enemies[1].direction = LEFT;
    state.enemies[1].width = 0.6f;
    state.enemies[1].enemylevel = ROUND3;
    ...
    for (int i = 0; i < SUCCESS_COUNT;i++) {
        ...
        state.win[i].position = glm::vec3(xpos, -3.5f, 0.0f);
        xpos += 0.5f;
    }
...
void Level3::Update(float deltaTime) {
    ...
    for (int i = 0; i < LEVEL3_ENEMY_COUNT;i++) {
        state.enemies[i].Update(deltaTime, &state.enemies[i], state.obstacles, LEVEL3_WATER_COUNT, state.map);
    }
    ...
    for (int i = 0; i < SUCCESS_COUNT; i++) { state.win[i].Update(0, &state.win[i], NULL, 0, state.map); }
}
void Level3::Render(ShaderProgram* program) {
    ...
    if (state.player->won) {
        Mix_PlayChannel(-1, state.player->successMusic, 0);  
        for (int i = 0; i < SUCCESS_COUNT; i++) {
